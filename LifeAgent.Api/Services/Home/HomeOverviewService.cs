...
public sealed class HomeOverviewService : IHomeOverviewService
{
                    ...
                    HasStrongOverlap(signalText, insight.Text)))
            {
                candidates.Add(new TodayFocusCandidate(
                    ToFocus(
                        ...
                        "查看计划",
                        "回头整理",
                        "/life/review?focus=recent_pattern",
                        "这条计划和近期反复出现的主题有关，适合回顾后再决定是否继续推进。",
                        "这条计划与最近多条生活记录中的重复主题相关。"),
                    700,
                    signal.CreatedAt));
            }
        ...
        foreach (var insight in insights)
        {
            ...
            candidates.Add(new TodayFocusCandidate(
                ToFocus(
                    ...
                    "查看回顾",
                    "回头整理",
                    "/life/review?focus=memory_related",
                    "这条发现和已确认记忆相呼应，适合回顾近期变化。",
        ...
        string explanation)
    {
        return new HomeOverviewTodayFocusDto
        {
            ...
            FollowUpHref = string.IsNullOrWhiteSpace(followUpHref) ? href : followUpHref,
