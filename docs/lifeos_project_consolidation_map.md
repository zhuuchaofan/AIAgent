...
GET /api/home/overview
  ...
  -> up to 3 personalized today-focus items
     ...
     with read-only statusGroup / followUpLabel / followUpHref / trackingReason
  -> compact contextSpine
     (threads, signals, nextBestLinks, context counts)
...
Today-focus follow-up links may point to `/reminders`, `/plans`, or
`/life/review?focus=...` so the next surface can preserve why the user arrived,
but those links remain navigation only.
...
GET /api/memory/review-inbox/preview
  ...
  -> separate stable, observing, and likely one-off signals
...
POST /api/life/chat
  ...
  -> returns read-only usedContext explaining which background was referenced
...
POST /api/life/review
  ...
  -> returns read-only continuityHints for next navigation
  -> no life_events write, no Memory write, no Reminder write, no Tool execution
...
- `/life/chat`: read-only life Q&A based on recent life records, pending
  ...
  show a compact used-context list that links back to Memory, reminders, plans,
  or recent review without writing any data.
- `/life/review`: read-only recent-life review generated by
  `POST /api/life/review`, with recent / today / week windows, optional
  evidence expansion from source life records, recent review themes, and
  continuity hints that point to related plans, confirmed memories, Memory
  Review, or Life Q&A without executing or writing data.
...
Life Q&A may use recent `life_events`, pending reminders, active Memory, and
plan signals as private context. It may return a read-only `usedContext` summary
for user-facing explainability, but it does not persist chat history or create
new data.
