...
- `/life/review`: read-only recent-life review generated by
  `POST /api/life/review`, with optional evidence expansion from source life
  records.
...
it does not persist chat history or create new data.
Life Review may use recent `life_events` and active Memory as private context,
but it returns only read-only review cards and evidence references.
