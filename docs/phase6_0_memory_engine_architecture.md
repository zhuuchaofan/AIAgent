...
>
> **Phase 6 进入状态：当前仅允许进入 Phase 6 design，不允许进入 Phase 6 implementation。**
>
...
5. **Agent Execution Contract Engine v1 (本地收敛完成)**：`AgentRunner` 已收敛为 `Resolve -> BuildContract -> BuildPlan -> Execute -> Validate -> Finalize` 的 pipeline 编排，`IntentResolver` / `ContractValidator` / `ActionExecutor` / `ResponseFinalizer` 职责已分离。
6. **Phase 6 implementation 仍 blocked**：最新 deployed revision 尚未完成 authenticated preview-only smoke；life_event planner behavior 尚未在最新 revision 上完成部署验证；RAG smoke 也需要在进入实现前通过。因此当前只允许进行 Phase 6 design。
...
flowchart TD
    ...
    subgraph MemoryEngine [Memory Engine (长期记忆引擎)]
        ...
        MemoryStore[(Conceptual Store: memories)]
    end
    ...
    Executor -->|future confirmed durable memory write| MemoryUpdater
    MemoryUpdater -->|写入/覆盖/合并 (future)| MemoryStore
...
| :--- | :--- | :--- | :---: | :---: | :---: | :--- |
| **Life Event** | 重大客观事实记录 | 2026-06-30 顺利完成 Phase 5 答辩 | 是 (导入/沉淀为长期记忆前必须确认) | 是 | 否 | 用于回顾、时间线分析、重大纪念日提醒 |
...
| **Knowledge** | 用户记录的长效方法论 | 遇到数据库死锁时的排查步骤文档 | 是 | 是 | 否 | 在用户发起相关技术咨询时直接召回作为上下文 |
| **Project** | 跨多天的复合任务体系 | 完成 life-agent 的 Phase 6.0 设计 | 是 | 否 | 完成后立即归档 | 每天总结时汇报该 Project 的进度与待办 |
| **Person** | 关系人肖像 | 导师张教授，喜好喝龙井茶 | 是 | 是 | 否 | 撰写邮件或准备会面时，自动提示该人物背景 |
| **Location** | 空间地理常去节点 | 朝阳区太阳宫南街甲1号（父母家） | 是 | 是 | 否 | 规划出行、日常通勤计划时提供建议 |
| **Routine** | 日常惯例与固定日程 | 每个工作日早晨 8:30 坐地铁通勤 | 是 | 是 | 否 | 在发生路线异常或延误时主动发出警报 |
| **Constraint** | **安全与红线约束限制** | 严重乳糖不耐受，禁用乳制品 | **是 (强制)** | 是 | 否 | **所有 proposedAction 在生成前必须经过此约束过滤校验** |
| **Temporary Context** | 临时的短期状态快照 | 本周正在上海出差，预计周五回京 | 是 | 否 | **是 (通常 1-7 天内)** | 临时覆盖日常 Routine，指导本周的活动建议 |
...
1. **Create (创建)**：Agent 在分析 RAG 对话、Timeline 记录或每日总结时，发现有价值的记忆实体，触发 `save_memory_preview` 提议。当前阶段仅允许生成 proposedAction，不允许直接写入 durable memory。
2. **Preview (预览)**：产生一个 `pending_confirm` 状态的记忆实体，并在前端的 `AgentPreview` 面板展示待确认卡片。
3. **Confirm (确认)**：未来实现中，用户点击同意后，才允许将 proposed memory 交给 durable memory write。若用户取消，则该 `pending` 动作直接作废，不写入物理记忆库。
4. **Store (存储)**：未来 confirmed write 才允许将记忆实体持久化至设计中的 memory store。同时进行文本分块，产生 768 维向量写入向量库，以便后续相似度召回。当前文档不创建 Firestore collection。
...
- **匹配规则**：当 LLM 检测到符合 12 种 Memory Taxonomy 中高价值记忆的特征（如“我最近开始...” “我总是...” “我以后不要...”）时，只能自动生成 `actionType = "save_memory_preview"` 的 proposedAction。

> 命名约束：planner / preview 阶段统一使用 `save_memory_preview`。只有未来 confirmed durable memory write 阶段，才允许设计 `save_memory` 或等价 durable memory write 动作。当前系统不具备真实 `save_memory` 写入能力。

### 7.2 必须用户确认的写入边界
为了防止 AI 基于幻觉或错误推理弄脏用户的记忆库，Phase 6 implementation 前，所有 durable memory write 都**必须经过 preview + confirm**：
1. **所有 Memory 类型的创建**：自动提取只能生成 proposedAction，不能直接写入 active memory。
2. **所有 Memory 类型的修改、合并、归档与删除**：必须由用户确认，或经过未来单独批准的 release gate 策略。
3. **Constraint（核心红线）的创建与修改**：涉及过敏源、健康限制、财务敏感词，必须强制确认。
4. **Goal（长期目标）的生命周期变更**：如建立考证目标、放弃买房目标等，必须强制确认。
5. **对已有记忆的冲突性更新**：如果新信息彻底推翻了已确认的旧记忆，必须强制确认。
...
* **定位不同**：
  ...
  * future `save_memory` / durable memory write 写入的是 **Cognitive Entity (认知实体)**。它是经过抽象和合并的长效认知（如“偏好：喜欢喝燕麦奶咖啡”），属于 Memory。当前阶段仅允许 `save_memory_preview` proposedAction。
* **协作流**：
  1. 用户在 Timeline 记录了多次“下午去跑步”（多个 `life_events`）。
  2. Weekly Agent 运行，读取 Timeline 后发现用户经常在下午跑步。
  3. Agent 触发 `save_memory_preview`，向用户提出一条 Habit 记忆提议：“您似乎习惯每周跑步，是否将‘跑步’保存为长期习惯偏好？”。
  4. 未来 confirmed durable memory write 被设计并通过 release gate 后，用户确认才允许该 Habit 进入 Memory。
...
未来实现可使用 Firestore 向量索引或等价 memory vector store 进行语义搜索，同时辅以 Metadata 的精确过滤（如 `status == "active"`）。当前阶段不创建 Firestore collection 或向量索引。对于召回的候选记忆，其排序得分计算如下：
...
- **Memory 是“认知沉淀”**：未来可由设计中的 `users/{userId}/memories` 或等价 memory store 构成，记录“我是个什么样的人，我有什么规则”（Structured patterns）。当前阶段不创建 Firestore collection。
- **两者的联动回馈机制**：
...
| **存储集合** | `users/{userId}/documents` & `chunks` | future `users/{userId}/memories` 或等价 memory store（当前不创建） |
...
```
                    1. 接收到用户请求 (Query)
                               │
            2. future Memory Retrieval (向量 + 属性混合检索)
                               │
            3. 将召回的 confirmed active 记忆装入 System Prompt context
                               │
            4. Planner 评估意图 (Intent Detection) 与约束 (Constraints)
                               │
         5. Planner 生成 proposedAction (如 create_life_event / save_memory_preview)
                               │
...
- Memory Retrieval 仍是 Phase 6 设计项，尚未实现。
- Memory proposedAction 只能使用 preview 语义，不能直接写入 active memory。
- `/api/agent/run` 不应新增 memory-specific endpoint；Phase 6.1 不暴露 API。
...
### 15.1 Memory Document (Conceptual: `users/{userId}/memories/{memoryId}`)

> 这是概念 schema，不代表当前已经创建 Firestore collection，也不代表当前可以写入 durable memory。
...
### 16.0 Release Gate 约束

在启动任何 Phase 6 implementation 前，必须先满足以下 release gate 前置条件：

1. authenticated preview-only smoke 必须在最新 API revision 通过。
2. life_event planner behavior 必须已部署并验证。
3. RAG smoke 必须通过。
4. 真实写入必须继续保持 disabled，除非单独 Release Gate 明确批准 canary 或 production enablement。
5. 不得修改 Cloud Run env、Firestore Rules、MCP 配置或部署策略来绕过上述验证。

| 开发步骤 | 目标名称 | 说明与任务 | 状态 | 完成标准 |
| :--- | :--- | :--- | :--- | :--- |
| **Phase 6.0** | Architecture Design | 完成核心架构、Taxonomy 和生命周期设计（本文件） | ✅ 本步完成 | 架构设计文档冻结并提交 Git |
| **Phase 6.1** | Taxonomy & Schema | 定义 Memory model / conceptual schema / validator / repository interface / fake repository / unit tests | 🟡 待启动 | 数据模型、validator、fake repository 单元测试通过；不暴露 API、不接真实 Firestore 写入、不改 Firestore Rules、不部署 |
| **Phase 6.2** | Proposal & Confirm | 设计并实现 `save_memory_preview` proposedAction 与确认契约 | 🟡 待启动 | 仅 preview + confirm 流程；durable memory write 仍需后续 gate |
...
* **不实现 Memory Engine**：Phase 6.0 和当前 readiness 状态只允许设计，不允许新增 memory 写入能力、Firestore collection、前端入口或 API endpoint。
...
0. **Phase 6 Readiness Blockers**：先完成最新 API revision 的 authenticated preview-only smoke、life_event planner 部署验证和 RAG smoke。
1. **Phase 6.1 (数据模型定义)**：只定义 Memory model、schema、validator、repository interface、fake repository 和单元测试；不暴露 API、不接真实 Firestore 写入、不改 Firestore Rules、不部署。
2. **Phase 6.2 (提议与用户确认)**：实现 `save_memory_preview` proposedAction 契约，打通 pending action 语义；durable memory write 仍保持 disabled，除非后续 Release Gate 批准。
