...
| Phase 6 | Memory Engine（长期记忆） | 🟡 Memory Value Loop + 个性化今日关注 | 用户可明确确认、查看和归档 durable Memory；Memory Review 会区分稳定、观察、一次性和已有相近记忆线索；首页以只读方式结合 Memory、提醒、近期模式和待判断线索生成今日关注与个人化今日简报；自动 Memory write 未启用 |
