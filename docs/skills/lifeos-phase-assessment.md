...
| Phase 6 | Memory Engine（长期记忆） | 🟡 Memory Value Loop 阶段 | Home AI 发现、Memory Review Inbox 与 RAG 只读上下文可展示候选线索；Review Inbox 状态可持久化，并会区分更稳定、观察中、一次性线索以降低记忆污染；已留着候选可由用户明确点击“记住”后写入 durable Memory；用户可查看、使用和归档已记住内容 |

**一句话结论**：当前项目已完成 RAG、Agent Preview、pending action 持久化、LifeEvent 最小真实写入与 Reminder 最小写入闸门，并进入 Memory Value Loop 阶段；当前收敛主线是 Unified Inbox + Memory Review + 我的记忆 + 生活问答，但提醒送达/调度、自动 Memory write、Tool Execution 均未开放。
