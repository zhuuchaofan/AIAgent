...
| Phase 6 | Memory Engine（长期记忆） | 🟡 Memory Value Loop + 个性化今日关注阶段 | Home AI 发现、Memory Review Inbox 与 RAG 只读上下文可展示候选线索；Review Inbox 状态可持久化，并会区分更稳定、观察中、一次性线索以降低记忆污染；已留着候选可由用户明确点击“记住”后写入 durable Memory；用户可查看、使用和归档已记住内容；首页可用活跃 Memory、提醒时效与近期模式生成只读、可解释的今日关注排序 |
