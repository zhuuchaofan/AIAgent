...
| Phase 6 | Memory Engine（长期记忆） | 🟡 Review-confirmed minimal write + management 阶段 | Home AI 发现、Memory Review Inbox 与 RAG 只读上下文可展示候选线索；Review Inbox 状态可持久化；已留着候选可由用户明确点击“记住”后写入 durable Memory；用户可查看和归档已记住内容 |

**一句话结论**：当前项目已完成 RAG、Agent Preview、pending action 持久化、LifeEvent 最小真实写入与 Reminder 最小写入闸门，并进入 Memory Review 确认写入与管理阶段；当前收敛主线是 Unified Inbox + Memory Review + Reminder 待处理体验，但提醒送达/调度、自动 Memory write、Tool Execution 均未开放。
