...
| Phase 6 | Memory Engine（长期记忆） | 🟡 Memory Value Loop + 个性化今日关注阶段 | Home AI 发现、Memory Review Inbox、最近回顾与 RAG 只读上下文可展示候选线索；Review Inbox 状态可持久化，并会区分更稳定、观察中、一次性和已有相近记忆线索以降低记忆污染；已留着候选可由用户明确点击“记住”后写入 durable Memory；用户可查看、编辑和归档已记住内容，并能看到只读质量提示以维护重复、过期或过泛记忆；首页可用活跃 Memory、提醒时效、计划线索、近期模式与待判断线索生成只读、可解释的今日关注排序、个人化今日简报和近期主线，并通过 `contextSpine` 输出可复用的个人上下文连续性读模型；首页会展示短缓存更新时间、手动刷新和后台更新状态，并可展开只读依据以提升个人上下文可信度；最近回顾可展示只读记忆/计划依据 |

**一句话结论**：当前项目已完成 RAG、Agent Preview、pending action 持久化、LifeEvent 最小真实写入与 Reminder 最小写入闸门，并进入 Memory Value Loop + 个性化今日关注阶段；当前收敛主线是 Unified Inbox + Daily Hub + 可感知刷新 + Memory Review + 我的记忆 + 生活问答，但提醒送达/调度、自动 Memory write、Tool Execution 均未开放。
