...
**详细清单**：见 `docs/phase3_5_smoke_test_checklist.md`（26 个验证项，6 大类）
...
| A2 | 未认证访问返回 401 | ✅ |
| A3 | 伪造 token 返回 401 | ✅ |
| E1 | Internal 无 token 拒绝 | ✅ 401 |
| E2 | Internal 伪造 token 拒绝 | ✅ 401 |
| F1 | Cloud Run 健康 | ✅ revision 00028-66b |
| F2 | Traffic 100% | ✅ |
| F3 | Git 状态干净 | ✅ |
| F4 | 单元测试 | ✅ 96 个通过 |
...
**验收标准**：
- [x] 冒烟清单文档化（`docs/phase3_5_smoke_test_checklist.md`）
- [x] 已执行轻量冒烟验证（9 项通过）
- [ ] 完整端到端冒烟（需手动登录 Web 执行）
...
| `docs/phase3_5_smoke_test_checklist.md` | 关键链路冒烟验证清单（详细版） |
| `CLAUDE.md` | 项目技术架构和开发指南 |

---

## 9. P0 完成总结（2026-06-28）

### P0 任务完成状态

| # | 任务 | 状态 | 提交 | 部署 |
|---|---|---|---|---|
| 2.1 | Firestore Security Rules | ✅ rules 文件已创建 | `2caca6e` | ⏸️ 未部署（跨项目 Auth 阻塞） |
| 2.2 | API Rate Limiting | ✅ 完成 | `39be375` | ✅ revision 00028-66b |
| 2.3 | 文件上传大小限制 | ✅ 完成 | `275cc3b` | ✅ revision 00027-q5p |
| 2.4 | 文档 chunk 数量限制 | ✅ 完成 | `4e0cfb7` | ✅ revision 00027-q5p |
| 2.5 | Gemini/Embedding 成本保护 | ✅ 完成 | `275cc3b` | ✅ revision 00027-q5p |
| 2.6 | 关键链路冒烟验证清单 | ✅ 完成 | 本次 | — |

### 已部署到线上的保护措施

| 保护 | 配置值 | 状态 |
|---|---|---|
| 文件大小限制 | FileValidator 10MB + Kestrel 15MB | ✅ 生效 |
| Chunk 数量上限 | MaxChunksPerDocument 200 | ✅ 生效 |
| 每日 LLM 配额 | 200 次/用户/日 | ✅ 生效 |
| 每日 Embedding 配额 | 500 次/用户/日 | ✅ 生效 |
| 每日文档处理配额 | 20 次/用户/日 | ✅ 生效 |
| API Rate Limiting | high-cost 10/min, auth-user 60/min, internal 20/min | ✅ 生效 |
| Firestore Security Rules | 默认 deny + isOwner 校验 | ⏸️ 文件已创建，未部署 |

### Phase 3.5 收尾判断

**P0 全部完成** ✅

Phase 3.5 可以收尾，进入 Phase 4 前的剩余事项：
